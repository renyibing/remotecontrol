...
void Websocket::Connect(const std::string& url, connect_callback_t on_connect) {
  // Only wss is supported when using a proxy
  if (https_proxy_) {
  ...
  // Only 'wss' and 'ws' schemes are supported
  ...
  // Ensure the constructor settings match the connection scheme
...
void Websocket::OnResolve(
  ...
  // Connect to the endpoints obtained from DNS lookup
  if (IsSSL()) {
...
void Websocket::ConnectProxy(const std::string& url,
  ...
  // Currently only the http + wss proxy combination is allowed
  ...
  // Resolve the DNS of the proxy server
  resolver_->async_resolve(
...
void Websocket::OnReadProxy(boost::system::error_code ec,
  ...
  // Create the wss stream, then perform a normal SSL handshake
