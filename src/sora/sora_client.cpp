...
SoraClient::~SoraClient() {
  ...
  // OnIceConnectionStateChange will be called here
  connection_ = nullptr;
}
...
void SoraClient::OnConnect(boost::system::error_code ec,
  ...
  connecting_wss_.erase(
      ...
      connecting_wss_.end());
  if (ec) {
    // If all connections fail, enter the reconnection flow
  ...
  if (ws_) {
    // If another connection has already completed, close it
...
void SoraClient::Redirect(std::string url) {
  ws_->Read([self = shared_from_this(), url](boost::system::error_code ec,
    ...
    auto on_close = [self, url](boost::system::error_code ec) {
      // Whether the close process succeeds or fails, continue processing
      URLParts parts;
      bool ssl;
    ...
    // After type: redirect, the server should be disconnected, so it is abnormal to complete the processing normally
    if (!ec) {
...
void SoraClient::DoSendConnect(bool redirect) {
  ...
  if (!config_.video) {
    // If video: false, set it as is
    json_message["video"] = false;
  } else if (config_.video && config_.video_codec_type.empty() &&
             config_.video_bit_rate == 0) {
    // If video: true, and other options are not set, set it to true
    json_message["video"] = true;
  } else {
    // For other cases, set the options correctly
...
void SoraClient::DoSendPong(
  ...
  if (dc_ && using_datachannel_ && dc_->IsOpen("stats")) {
    // If DataChannel is available, send via DataChannel with type: stats
...
void SoraClient::DoSendUpdate(const std::string& sdp, std::string type) {
  ...
  if (dc_ && using_datachannel_ && dc_->IsOpen("signaling")) {
    // If DataChannel is available, send via DataChannel
...
std::shared_ptr<RTCConnection> SoraClient::CreateRTCConnection(
  ...
  // macOS Simulcast: Somehow the resolution falls infinitely, so disable cpu_adaptation to avoid it
...
void SoraClient::OnRead(boost::system::error_code ec,
  ...
  // When the read process is canceled for writing, this error occurs, so it is not treated as an error
  if (ec == boost::asio::error::operation_aborted)
    return;

  if (ec) {
    // First close WS and DC, and then reconnect later
    ReconnectAfter();
    Close([]() {});
  ...
  if (type == "redirect") {
    ...
    Redirect(location);
    // In Redirect, the next Read is done, so return here
    return;
  } else if (type == "offer") {
    // Remember the labels of compressed data sent by the Data Channel
    {
    ...
    connection_->SetOffer(sdp, [self = shared_from_this(), json_message]() {
      boost::asio::post(self->ioc_, [self, json_message]() {
        ...
        // For simulcast, tracks need to be added after setRemoteDescription of the offer is complete, so initialize here
        ...
        if (self->config_.simulcast &&
          ...
          // Convert the contents of the "encodings" key to webrtc::RtpEncodingParameters
          ...
          std::string mid;
          {
            // TODO(melpon): Consider the possibility that mid may be missing for a while, but eventually make it mandatory
            auto it = json_message.as_object().find("mid");
            if (it != json_message.as_object().end()) {
              const auto& midobj = it->value().as_object();
              // If video: false, check if video field is missing
    ...
    connection_->SetOffer(sdp, [self = shared_from_this(), answer_type]() {
      boost::asio::post(self->ioc_, [self, answer_type]() {
        ...
        // The encoding parameter information is cleared, so set it again
        if (self->config_.simulcast) {
    ...
    // Start communication via Data Channel
    using_datachannel_ = true;

    // If ignore_disconnect_websocket == true, close WS
...
void SoraClient::OnMessage(
  ...
  // For labels that need to be handled, do nothing
...
// WebRTC callbacks
// These are coming from a different thread, so handle with care
void SoraClient::OnIceConnectionStateChange(
    webrtc::PeerConnectionInterface::IceConnectionState new_state) {
  RTC_LOG(LS_INFO) << __FUNCTION__ << " state:" << new_state;
  // In the destructor, shared_from_this does not work, so ignore it
  if (destructed_) {
    return;
  }
