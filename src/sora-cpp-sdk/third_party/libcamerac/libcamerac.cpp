...
// Convert the string value of an enum control to a number.
static bool parse_enum_control(const std::string& name,
                               const std::string& value_str,
                               int32_t& result) {
  // Mapping of major enum controls.
  if (name == "AfMode") {
    ...
    // Other enums are parsed as numbers.
...
int libcamerac_ControlList_set_by_name(libcamerac_Camera* camera,
  ...
  // Search for the control by name from the camera's controls.
  ...
  if (!controlId) {
    return -1;  // The control is not found.
  }
  ...
  try {
    ...
    switch (controlId->type()) {
      ...
      case libcamera::ControlTypeInteger32: {
        if (is_array) {
          // For int32 arrays (e.g. FrameDurationLimits).
          ...
          // Since it may be an enum, try to parse it as a string first.
          int32_t val;
      ...
      case libcamera::ControlTypeInteger64: {
        if (is_array) {
          // For int64 arrays (e.g. FrameDurationLimits).
          auto tokens = split_string(value_string, ',');
      ...
      case libcamera::ControlTypeFloat: {
        if (is_array) {
          // For float arrays (e.g. ColourGains).
          auto tokens = split_string(value_string, ',');
      ...
      case libcamera::ControlTypeRectangle: {
        // For Rectangle type (e.g. ScalerCrop).
        ...
        if (is_array) {
          // For Rectangle arrays (e.g. AfWindows).
          // Separate multiple Rectangles with semicolons.
      ...
      default:
        return -2;  // The type is not supported.
    }

    list->set(controlId->id(), value);
    return 0;
  } catch (...) {
    return -3;  // Value parsing error.
  }
}
