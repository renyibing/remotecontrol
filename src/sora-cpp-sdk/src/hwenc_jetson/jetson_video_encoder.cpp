...
// To prevent output to stdout and stderr
...
int32_t JetsonVideoEncoder::InitEncode(const webrtc::VideoCodec* codec_settings,
  ...
  if (codec_settings->codecType == webrtc::kVideoCodecH264) {
    ...
    // codec_settings->AV1() does not have a key frame setting, but 
    // key_frame_interval_ itself must be initialized with some value,
    // otherwise it will become an undefined value, so put in a random value.
    // It will become an undefined value, so put in a random value.
    key_frame_interval_ = 3000;
  }
...
int32_t JetsonVideoEncoder::JetsonConfigure() {
  ...
  if (codec_.codecType == webrtc::kVideoCodecH264) {
    ...
    // Required, otherwise the frame rate will not be output with H264
    ...
    // V4L2_ENC_HW_PRESET_ULTRAFAST is the recommended value, but MEDIUM is also OK for Nano and AGX
    // NX does not output the frame rate unless V4L2_ENC_HW_PRESET_FAST is specified
    ...
    // 30fps will not be output unless V4L2_ENC_HW_PRESET_ULTRAFAST is specified
    ret = encoder_->setHWPresetType(V4L2_ENC_HW_PRESET_ULTRAFAST);
    INIT_ERROR(ret < 0, "Failed to setHWPresetType");
  } else if (codec_.codecType == webrtc::kVideoCodecVP9) {
    // QP:150 is the lower limit for 30fps. If you lower it further, 30fps will be broken.
    ret = encoder_->setQpRange(QP_RETAIN_VAL, 150, QP_RETAIN_VAL, 150,
                               QP_RETAIN_VAL, 150);
    INIT_ERROR(ret < 0, "Failed to setQpRange");

    // V4L2_ENC_HW_PRESET_ULTRAFAST is the recommended value, but SLOW also does not change the frame rate.
  ...
  /* This means the encoder is initialized.
     The encoder is initialized, so the bitrate should be set to the initial target value, not the adjusted rate.
     Initialize the BitrateAdjuster.*/
...
void JetsonVideoEncoder::SetBitrateBps(uint32_t bitrate_bps) {
  ...
  // The VP9 setBitrate does not look at the set frame rate,
  // so it seems to look at the bps at 60fps, so adjust the bps here.
...
int32_t JetsonVideoEncoder::Encode(
  ...
  if (use_native_) {
    ...
    for (int i = 0; i < MAX_PLANES; i++) {
      ...
      // I thought it might be necessary to NvBufSurfaceDestroy here, but after checking the sample code below, it seems unnecessary.
      // Reference: jetson_multimedia_api/samples/01_video_encode/video_encode_main.cpp
...
int32_t JetsonVideoEncoder::SendFrame(
  ...
  if (codec_.codecType == webrtc::kVideoCodecH264) {
    ...
    // VP8, VP9, AV1 have an IVF header in the encoded frame, so remove it.
    ...
    if (codec_.codecType == webrtc::kVideoCodecAV1) {
      // Since JetPack 5.1.1, the sequence header (OBU_SEQUENCE_HEADER) is no longer included in the encoded frame of AV1.
      // If you do not set the sequence header for the key frame, the decoder will not be able to decode it from the middle.
      // Therefore, save the first sequence header and insert it before the key frame to correspond.

      // The first 2 bytes of the buffer are 0x12 0x00, which is OBU_TEMPORAL_DELIMITER, so ignore it and look at the next OBU.
      if (buffer[2] == 0x0a) {
        // 0x0a is OBU_SEQUENCE_HEADER, so save this OBU.
        // obu_size is actually LEB128, but it is not likely to be more than 128 bytes, so get it as 1 byte.
        int obu_size = buffer[3];
        obu_seq_header_.resize(obu_size + 2);
        memcpy(obu_seq_header_.data(), buffer + 2, obu_size + 2);
      }

      // If the frame is marked as a key frame but does not contain a sequence header,
      // insert the sequence header before the frame.
    ...
    if (codec_.codecType == webrtc::kVideoCodecVP8) {
      ...
      // I could not know if nonReference is true or false.
