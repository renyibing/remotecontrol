...
std::optional<std::vector<V4L2Device>> EnumV4L2CaptureDevices() {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    V4L2Device device;
    // Extract the XX from videoXX and use it as the index.
    // Since video10 comes before video2 when sorting as a string,
  ...
  for (const auto& cand_device : candidate_devices) {
    ...
    // Only devices that can capture video are targeted.
    ...
    while (ioctl(fd, VIDIOC_ENUM_FMT, &fmt) == 0) {
      ...
      // Enumerate the resolutions supported by each format.
      ...
      while (ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmsize) == 0) {
        ...
        // Enumerate the frame rates supported by each resolution of each format.
    ...
    // Devices that cannot enumerate any formats are excluded.
