...
int32_t V4L2VideoCapturer::Init() {
  ...
  if (config_.video_device.empty()) {
    ...
    // Search for a device that matches the specified device name.
    bool found = false;
...
int32_t V4L2VideoCapturer::StartCapture() {
  ...
  // Search for the video formats supported in order.
  // If no specific specification is given, MJPEG is preferred for resolutions larger than VGA, and I420 is preferred for resolutions smaller than VGA.
  int nFormats = 0;
  ...
  // Convert FOURCC to webrtc::VideoType.
  ...
  if (ioctl(_deviceFd, VIDIOC_G_PARM, &streamparms) < 0) {
    ...
    // Check if the capability flag is set to V4L2_CAP_TIMEPERFRAME.
    if (streamparms.parm.capture.capability & V4L2_CAP_TIMEPERFRAME) {
      // The driver supports the feature. Set the required framerate.
      memset(&streamparms, 0, sizeof(streamparms));
      streamparms.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  ...
  // Hardcode the value based on the frame size.
...
bool V4L2VideoCapturer::CaptureProcess() {
  ...
  retVal = select(_deviceFd + 1, &rSet, NULL, NULL, &timeout);
  {
    ...
    if (_captureStarted) {
      ...
      // Some cameras (DELL WB7022) may send invalid data.
      // If this is sent to a hardware JPEG decoder, it will crash.
      // Since the JPEG header starts with the SOI marker 0xffd8, check it to avoid crashing.
...
void V4L2VideoCapturer::OnCaptured(uint8_t* data, uint32_t bytesused) {
  ...
  if (_captureVideoType == webrtc::VideoType::kNV12) {
    ...
    // If it is YUY2, convert it to NV12.
    webrtc::scoped_refptr<webrtc::NV12Buffer> nv12_buffer =
        webrtc::NV12Buffer::Create(_currentWidth, _currentHeight);
    nv12_buffer->InitializeData();
    ...
    // If it is not NV12 or YUY2, convert it to I420.
