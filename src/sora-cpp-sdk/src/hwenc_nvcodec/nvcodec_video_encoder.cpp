...
NvCodecVideoEncoderImpl::NvCodecVideoEncoderImpl(
  ...
  // The following is only for getting the device name.
...
int32_t NvCodecVideoEncoderImpl::Encode(
  ...
  for (NvEncOutputFrame& output : v_packet_) {
    ...
    if (codec_ == CudaVideoCodec::AV1) {
      // IVF header is attached, so remove it.
    ...
    // Treat IDR or I frame as a key frame.
    ...
    if (codec_ == CudaVideoCodec::H264) {
      ...
      // If layer_frames[0] is invalid, access violation occurs, but it is basically not invalid.
...
void NvCodecVideoEncoderImpl::SetRates(
  ...
  // Disable the layer when the bitrate is 0.
  if (svc_controller_) {
  ...
  try {
    ...
    if (codec == CudaVideoCodec::H264) {
      ...
      encode_config.encodeCodecConfig.av1Config.idrPeriod =
          NVENC_INFINITE_GOPLENGTH;
      // Prevent the key frame from not having size information.
...
bool NvCodecVideoEncoder::IsSupported(std::shared_ptr<CudaContext> cuda_context,
  ...
  try {
    ...
    // Check if cuda and nvcuvid are needed for Linux.
#ifdef __linux__
    ...
    // Check if the encoder can be created.
#ifdef _WIN32
    ComPtr<IDXGIFactory1> idxgi_factory;
