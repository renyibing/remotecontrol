...
// Class to receive stats callback
...
// Class to handle CreateSessionDescriptionObserver callback as a function object
...
// Class to handle SetSessionDescriptionObserver callback as a function object.
...
void RTCConnection::CreateOffer(OnCreateSuccessFunc on_success,
  ...
  // CreateOffer is only done by Ayame, so we create DataChannels in the Offer case here
  // Since Momo does not ReOffer, this is not a problem
  ...
  if (data_manager != nullptr) {
    ...
    // Create data channels for remote control: input-reliable / input-rt
    // Reliable channel: ordered + reliable
    webrtc::DataChannelInit input_rel_cfg;
    input_rel_cfg.ordered = true;
    auto dc_rel = connection_->CreateDataChannelOrError("input-reliable", &input_rel_cfg);
    if (!dc_rel.ok()) {
      RTC_LOG(LS_ERROR) << "CreateDataChannel(input-reliable) failed: "
                        << dc_rel.error().message();
    } else {
      data_manager->OnDataChannel(dc_rel.MoveValue());
    }
    // Low-latency channel: unordered + lossy (set maxRetransmits=0 to disable retransmission)
    webrtc::DataChannelInit input_rt_cfg;
    input_rt_cfg.ordered = false;
    input_rt_cfg.maxRetransmits = 0;
    auto dc_rt = connection_->CreateDataChannelOrError("input-rt", &input_rt_cfg);
    if (!dc_rt.ok()) {
      RTC_LOG(LS_ERROR) << "CreateDataChannel(input-rt) failed: "
                        << dc_rt.error().message();
    } else {
      data_manager->OnDataChannel(dc_rt.MoveValue());
    }
  }
...
void RTCConnection::CreateAnswer(OnCreateSuccessFunc on_success,
                                 OnCreateFailureFunc on_failure) {
  // Create data channels on the response side (to avoid dependency on the other party)
  RTCDataManager* data_manager = observer_->DataManager();
  if (data_manager != nullptr) {
    webrtc::DataChannelInit input_rel_cfg;
    input_rel_cfg.ordered = true;
...
void RTCConnection::SetEncodingParameters(
  ...
  if (mid.empty()) {
    // TODO(melpon): mid is now available, so we will eventually remove this implementation

    // After setRD, the direction becomes recv only.
    // Currently, sender.track.streamIds cannot be obtained, so comparison with the connection ID is also not possible.
    // When the video upstream is held, the first video type transceiver is determined as the transceiver to be sent by oneself.
    for (auto transceiver : connection_->GetTransceivers()) {
