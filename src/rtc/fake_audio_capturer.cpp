...
void FakeAudioCapturer::AudioThread() {
  // Generate audio data every 10ms
  ...
  while (!stop_audio_thread_.load()) {
    ...
    // Maintain accurate 10ms intervals
    next_time += interval;
...
void FakeAudioCapturer::GenerateBeep(std::vector<int16_t>& buffer,
  ...
  const double amplitude = 16000;  // Volume (half of the maximum 32767)
