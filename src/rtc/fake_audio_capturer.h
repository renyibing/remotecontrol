...
class FakeAudioCapturer : public webrtc::AudioDeviceModule {
 public:
  struct Config {
    ...
    int channels = 1;
    int fps = 30;  // Synchronize with the video FPS
  };
  ...
  // Trigger the beep when the video circle completes
  void TriggerBeep();
 ...
 private:
  ...
  double beep_phase_ = 0.0;           // sin phase
  const int beep_duration_ms_ = 100;  // Beep duration (milliseconds)
  const int beep_frequency_ = 1000;   // Beep frequency (Hz)
};
