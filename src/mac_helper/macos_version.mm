...
// Check the OS name from TARGET_OS_*.
// The architecture can also be determined from the macro, but it is not needed because it is determined at runtime using uname

// The following information is copied from /Library/Developer/CommandLineTools/SDKs/MacOSX10.15.sdk/usr/include/TargetConditionals.h
...
// Currently, it is only necessary to know if it is Mac, so split by TARGET_OS_MAC
#if TARGET_OS_MAC
  return "macOS";
#else
  return "Unknown OS";
...
std::string MacosVersion::GetOSVersion() {
  // Get a string like "Version 10.8.2 (Build 12C60)"
