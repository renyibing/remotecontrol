...
def install_deps(
    ...
    with cd(BASE_DIR):
        ...
        # Build sysroot using multistrap
        if platform.target.os == "jetson" or platform.target.os == "raspberry-pi-os":
            ...
            # The hash value of the conf file is the version
        ...
        # Windows uses MSVC, so it is not needed
        # macOS uses Apple Clang, so it is not needed
        ...
        # Blend2D (for fake capture device)
        # macOS and Ubuntu x86_64 only install
