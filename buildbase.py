# buildbase.py is a template file for the build script.
#
# Copy it to your own repository and use it.
#
# The original file is located in the following repository:
...
# Execute command capturing standard output. Same as shell `cmd ...` or $(cmd ...)
...
def download(
    ...
    if os.path.exists(output_path):
        # Even if the file already exists, perform hash check
        if expected_sha256 is not None:
            try:
                verify_sha256(output_path, expected_sha256)
            except ValueError:
                # If the hash is inconsistent, delete the file and try to download again
    ...
    try:
        ...
        # After downloading, perform hash check
        if expected_sha256 is not None:
            try:
                verify_sha256(output_path, expected_sha256)
        ...
        # To avoid leaving garbage, delete the file
        if os.path.exists(output_path):
...
def verify_sha256(file_path: str, expected_sha256: str):
    ...
    with open(file_path, "rb") as f:
        # To improve memory efficiency, read in chunks
...
# Return all file paths under dir as relative paths from dir2
...
# Check if the archive is stored in a single directory.
#
# If it is stored in a single directory, return the directory name.
# If not, return None.
def _is_single_dir(infos, get_name, is_dir) -> Optional[str]:
    ...
    # Both are separated by /, but in the case of a directory, there is a difference in whether / is appended or not.
    dirname = None
    for info in infos:
        name = get_name(info)
        n = name.rstrip("/").find("/")
        if n == -1:
            # A file exists in the root directory
            if not is_dir(info):
                return None
            dir = name.rstrip("/")
        else:
            dir = name[0:n]
        # Two or more directories exist in the root directory
...
# After extracting, assign file attributes
...
# Expand zip or tar.gz files.
#
# The destination directory is {output_dir}/{output_dirname},
# and if the destination directory already exists, it is deleted.
#
# If the content of the archive is a single directory,
# it is expanded as if it did not exist.
#
# For example, if the content of the archive libsora-1.23.tar.gz is
# ['libsora-1.23', 'libsora-1.23/file1', 'libsora-1.23/file2']
# then extract('libsora-1.23.tar.gz', 'out', 'libsora') will output
# - out/libsora/file1
# - out/libsora/file2
# .
#
# Also, if the content of the archive libsora-1.23.tar.gz is
...
# NOTE(enm10k): I wanted to use shutil.copytree with dirs_exist_ok=True added in Python 3.8, 
# but the version of Python installed on the Windows runner (widnwos-2019) of GitHub Actions was too old to use it.
# When I set up Python 3.8 with actions/setup-python and built Lyra, it failed to build, so I created this function.
# If the Windows runner is updated and the version of Python is 3.8 or higher, this function may not be needed.
...
def replace_vcproj_static_runtime(project_file: str):
    # Why MSVC_STATIC_RUNTIME doesn't work and DLL runtime is used
    # Change to use static runtime for the generated project
...
def build_webrtc(platform, local_webrtc_build_dir, local_webrtc_build_args, debug):
    with cd(local_webrtc_build_dir):
        ...
        # I don't want to increase the include directory, so copy __config_site to the libc++ directory.
...
def get_webrtc_info(
    ...
    # Allow specifying the custom path of WebRTC via environment variables.
    # Example: set MOMO_WEBRTC_DIR=D:/WebRTC/webrtc-m138
    custom_webrtc_dir = os.environ.get("MOMO_WEBRTC_DIR")
    if custom_webrtc_dir and os.path.exists(custom_webrtc_dir):
        # Assume the standard WebRTC build structure for Windows.
        # include: <MOMO_WEBRTC_DIR>/src
        # library: <MOMO_WEBRTC_DIR>/out/ReleaseWithNoDebug/obj
        webrtc_src_dir = os.path.join(custom_webrtc_dir, "src")
        webrtc_lib_dir = os.path.join(custom_webrtc_dir, "out", "ReleaseWithNoDebug", "obj")

        if os.path.exists(webrtc_src_dir) and os.path.exists(webrtc_lib_dir):
            return WebrtcInfo(
                version_file=os.path.join(webrtc_install_dir, "VERSIONS"),
                deps_file=os.path.join(webrtc_install_dir, "DEPS"),
                webrtc_include_dir=webrtc_src_dir,
                webrtc_source_dir=webrtc_src_dir,
                webrtc_library_dir=webrtc_lib_dir,
                clang_dir=os.path.join(install_dir, "llvm", "clang"),
                libcxx_dir=os.path.join(install_dir, "llvm", "libcxx"),
                libcxxabi_dir=os.path.join(webrtc_src_dir, "third_party", "libc++abi", "src"),
            )
...
# Patch to fix the following issues.
#
...
def build_and_install_boost(
    ...
    archive = download(
        # Mirror for Shiguredo to avoid overloading the official site.
        f"https://oss-mirrors.shiguredo.jp/boost_{version_underscore}.tar.gz",
        # Mirror for Boost official.
    ...
    with cd(os.path.join(build_dir, "boost")):
        ...
        # If Windows and Boost 1.85.0, apply the patch.
        ...
        if target_os == "iphone":
            ...
            for arch, sdk in IOS_BUILD_TARGETS:
                # Use clang++ of Xcode.
                # However, if cxx is specified, use that instead.
            ...
            # Build Android using android-ndk.
            # However, if cxx is specified, use that instead.
            with open("project-config.jam", "w", encoding="utf-8") as f:
                bin = os.path.join(
...
def install_rootfs(version, install_dir, conf, arch="arm64"):
    ...
    # Replace absolute path symbolic links with relative paths.
    for dir, _, filenames in os.walk(rootfs_dir):
        for filename in filenames:
            ...
            if not os.path.isabs(target):
                continue
            # By adding rootfs_dir to the beginning, make the absolute path correct from the outside of rootfs.
            targetpath = rootfs_dir + target
            # Whether the absolute path of the reference exists.
    ...
    # Because the symbolic link is not registered, create it.
...
def install_android_ndk(version, install_dir, source_dir, platform="linux"):
    ...
    if platform == "darwin":
        ...
        # The result should be like the following, so extract only /Volumes/Android NDK r26 from here.
        # /dev/disk4              GUID_partition_scheme
        # /dev/disk4s1            EFI
        # /dev/disk4s2            Apple_HFS                       /Volumes/Android NDK r26
        volume = cap.split("\n")[-1].split("\t")[-1]
        # There should be an app like AndroidNDK10792818.app.
        app = glob.glob("AndroidNDK*.app", root_dir=volume)[0]
        # Copy the NDK directory.
        cmd(
            [
                "cp",
                "-r",
...
def install_llvm(
    ...
    mkdir_p(llvm_dir)
    with cd(llvm_dir):
        # Hit update.py of tools to get the specific version of clang binary.
        ...
        # Use the specific version of libcxx.
        git_clone_shallow(libcxx_url, libcxx_commit, "libcxx")

        # Get the specific version of buildtools for __config_site.
        ...
        # Copy __assertion_handler.
        # Background: https://source.chromium.org/chromium/_/chromium/external/github.com/llvm/llvm-project/libcxx.git/+/1e5bda0d1ce8e346955aa4a85eaab258785f11f7
        shutil.copyfile(
            # NOTE(enm10k): Initially, I copied default_assertion_handler.in, but there is __assertion_handler generated from default_assertion_handler.in in buildtools, so I copied it.
...
def install_cmake(version, source_dir, install_dir, platform: str, ext):
    ...
    # When using a custom CMake on Android, there is a problem that the path to ninja cannot be found, so a symlink is pasted in the same directory.
...
def install_sdl2(
    ...
    with cd(sdl2_build_dir):
        ...
        if platform == "windows":
            ...
            # Because the ON/OFF of each feature depends on whether it is installed in the system, all ON/OFF are explicitly specified so that the same installation is performed in any environment.
            cmake_args += [
                "-DSDL_ATOMIC=OFF",
                "-DSDL_AUDIO=OFF",
                "-DSDL_VIDEO=ON",
    ...
    with cd(sdl3_build_dir):
        ...
        if platform == "windows":
            cmake_args += [
                ...
                "-DSDL_JOYSTICK=ON",
                "-DSDL_HAPTIC=ON",
                # On GitHub Actions, gameinput.h does not exist, but check_c_source_compiles() succeeds.
                # Why check_c_source_compiles() succeeds.
                # Force disable HAVE_GAMEINPUT_H=0.
                "-DHAVE_GAMEINPUT_H=0",
            ]
        elif platform == "macos":
            # Because the ON/OFF of each feature depends on whether it is installed in the system, all ON/OFF are explicitly specified so that the same installation is performed in any environment.
...
def install_openh264(version, source_dir, install_dir, is_windows):
    ...
    with cd(os.path.join(source_dir, "openh264")):
        if is_windows:
            # Windows doesn't have make, so copy manually
            # install-headers:
...
def install_protobuf(version, source_dir, install_dir, platform: str):
    ...
    # Why the executable attribute is lost, so add it
...
def install_protoc_gen_jsonif(version, source_dir, install_dir, platform: str):
    ...
    # Copy the binary of the current environment to <install-path>/bin
    shutil.copytree(
...
# Prevent errors when trying to generate executable files in cross-compilation environments like iOS and Android
#
# Based on v1.64.1
#
# MEMO: Command to get all diffs including submodules in gRPC
...
def install_amf(version, install_dir):
    # AMF is more of a convenience function + sample code than a library, so
    # here we only download the source and let the user reference the files as needed.
...
def get_windows_osver():
    ...
    # For non-Windows environments, reportAttributeAccessIssue is reported, so ignore it
...
def get_clang_version(clang):
    ...
    # version_str is a string like the following, so extract the version from it
    #
    ...
    #
    # For Android, the string is like the following:
    #
    ...
    # Extract the clang version from the string
...
def fix_clang_version(clang_dir, clang_version):
    # Check if <clang_dir>/lib/clang/<clang_version>/include or
    # <clang_dir>/lib64/clang/<clang_version>/include exists,
    # and if not, adjust clang_version to the existing version
    #
    # There are cases where <clang_dir>/lib/clang/16.0.0/include and
    # <clang_dir>/lib/clang/16/include exist.
...
# Internally uses os.path.abspath(), which depends on the current directory,
# so the current directory must be changed before ArgumentParser.parse_args() is executed.
# Also, the `--sora-args` option must use `=` like `--sora-args='--test'`, not `--sora-args '--test'`.
# If you use spaces, the option starting with a hyphen will not be correctly interpreted.
...
# The same notes as add_sora_arguments.
